...
public class WireTransaction extends TransferTransaction {
    ...
    public WireTransaction(double value, int tracingNumber, UserPerson receiver, String receiverInfo, String sign, SimpleUser sender, Boolean isReceiver, double accountBalance) {
        super(value, tracingNumber, receiver, false, receiverInfo, sign, sender, isReceiver, accountBalance);
    ...
    public void completeTransaction(CentralBank centralBank, NeoBank neoBank) {
        ...
        this.getSender().getAccount().addTransaction(new WireTransaction(removeValue, this.getTracingNumber(), this.getReceiver(), this.getReceiverInfo(), this.getSign(), this.getSender(), false, getSender().getAccount().getBalance()));
